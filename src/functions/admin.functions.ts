 ...
 * `src/functions/history.functions.ts` do. It assembles the {@link AdminOverview}
 * the Admin_Page renders: the signed-in email, the Member_Registry and
 ...
 *
 * The `/admin` route guard is UX; this server function is the boundary
 * (Requirement 6.7). `getAdminOverview` runs
 * {@link requireAdminForServerFn} over a fresh authorization decision **before
 * it reads any Admin_Page value**, so a direct `fetch` from an anonymous caller
 * gets a `NOT_AUTHENTICATED` envelope (never a redirect — the caller cannot
 * follow one into an HTML sign-in document), a member gets `NOT_AUTHORIZED`, and
 * an account whose role could not be read gets `AUTHORIZATION_UNKNOWN`. No
 * rejection carries an Admin_Page value, and no count is read before the guard
 ...
 * singletons.
 *
 * Exported and injectable for the same reason the roster and history surfaces
 * export their envelope operations separately from the `createServerFn` wrapper:
 * `createServerFn(...).handler(fn)` cannot be invoked directly in a unit test,
 * so the logic lives in a plain function that takes its collaborators.
 */
 ...
 *
 * Exported and taking its collaborators as a parameter so the guard boundary
 * (Requirement 6.7) and the partial-page assembly (Requirements 8.5, 8.6, 8.9)
 * can be exercised without booting the framework.
 *
 ...
 * it is the per-request Admin_Role check {@link requireAdminForServerFn} makes
 * inside {@link buildAdminOverview}, so this boundary refuses an anonymous,
 * member, or unknown caller with an envelope rather than a redirect.
 */
