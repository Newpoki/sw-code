...
2. WHEN a result row holds the Member_Outcome `UPSTREAM_ERROR` or `TRANSPORT_ERROR`, THE Web_Client SHALL display the response message of the Upstream_Result in that row as literal text, SHALL remove every HTML tag from that message without interpreting any of it as markup, and SHALL limit the displayed message to its first 500 characters after that removal.
